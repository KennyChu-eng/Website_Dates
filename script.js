...
function growYesBtn() {
  ...
  document.getElementById('yes-btn').style.boxShadow =
    `0 ${4 + next * 6}px ${20 + next * 14}px rgba(201,127,138,${0.3 + next * 0.15})`;

}

function runAway() {
  // On the very first click, move the button to <body> so it is never
  // inside a CSS-transformed ancestor (which breaks position:fixed).
  // We snapshot its viewport position first so it doesn't jump.
  if (noAttempts === 0) {
...
function reset() {
  ...
  noAttempts = 0;
  // Put the No button back inside the hero buttons row
  noBtn.style.cssText = '';
  ...
  document.getElementById('hero-sub').textContent = "I'd love to take you somewhere truly special.\nWhat do you say?";
  goTo(0);
}
